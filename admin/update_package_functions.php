        ...
        if (in_array($fileActualExt, $allowed)) {
            if ($fileError === 0) {
                if ($fileSize < 50000000) {
                    ...
                    $sql = "UPDATE `Package` SET package_name` = ?, `package_subject` = ?, `package_des` = ?, `amount` = ?, `image` = ? WHERE `id` = ?;";
                    $stmt = mysqli_prepare($conn, $sql);
                    mysqli_stmt_bind_param($stmt, "sssisi", $title, $subject, $description, $amount, $fileDestination, $pkg_id);
